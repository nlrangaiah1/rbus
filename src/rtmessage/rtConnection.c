  ...
  rtMessageInfo** res, int32_t timeout, int flags)
{
  ...
  while(1)
  {
    ...
    rtLog_Info("%s ,Req:%s ,tid:%d ,topic:%s ,sequence_number:%d ,err:%d ,con->read_tid:%d ,con->send_buffer:%s ,con->recv_buffer:%s ,application_name=%s",__FUNCTION__,pReq,tid,topic,sequence_number,err,con->read_tid,con->send_buffer,con->recv_buffer,con->application_name);  
    if (err != RT_OK)
    {
      ret = err;
